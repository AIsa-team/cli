...
| Item | Value |
| --- | --- |
| Version | `0.6.2` (release target) |
| Command surface | 22 root help entries including implicit `help`; `api` is `list`/`show` only |
| Registry latest | `0.6.1` on `https://registry.npmjs.org` (baseline at this preparation; recheck before tagging) |
...
`CHANGELOG.md` `## [0.6.2]` must agree. Confirm with
...
# baseline at this preparation: 0.6.1 — recheck before tagging

git checkout main
git pull origin main
# Confirm this commit is the reviewed merge of the 0.6.2 candidate.
node -p "require('./package.json').version"   # 0.6.2
grep -E '^export const VERSION' src/constants.ts   # "0.6.2"
...
```bash
node scripts/package-smoke.mjs --tarball /path/to/aisa-one-cli-0.6.2.tgz
```
...
Watch the Release workflow. Success is `0.6.2` on
