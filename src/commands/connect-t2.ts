...
(function () {
  ...
  function hydrate(s) {
    ...
    // Coming back from the sign-in tab is not the same as arriving late.
    //
    // The reader left this page for exactly one row — the sign-in — and that
    // row's outcome is the only thing they went away to get. Adopting it in
    // silence with everything else meant the trip they had just taken had no
    // visible result: the checklist was simply further along than they left
    // it, and the one line that says the key is theirs went by unread.
    //
    // So the return resumes at the sign-in: everything above it is adopted
    // at once (no replay from the top), the sign-in itself lands with its own
    // tick and its own sentence, and only then does the run move on. This
    // holds even for a run that finished while they were away — the rows are
    // paced out rather than snapped to a finished checklist.
    var resumeAt = -1;
    if (CAME_BACK) {
      (s.steps || []).forEach(function (x, i) { if (resumeAt < 0 && x.id === "signin") resumeAt = i; });
      // Consume the marker: a manual reload later is an arrival, not a return,
      // and should not replay a sign-in that happened minutes ago.
      if (history.replaceState) history.replaceState(null, "", location.pathname);
    }
    if ((settled || VIEW === "done") && resumeAt < 0) {
      ...
      // everything already settled is adopted at once and the animation picks
      // up at the row that is actually working. The one exception is the
      // return from the sign-in (resumeAt), which is not an arrival: see the
      // note where resumeAt is computed.
      if (CAME_BACK) {
        var bf = $("#backfrom");
        if (bf) {
          // Says the one thing the checklist cannot: the trip is over and
          // this is the same setup they left. The receipt — that a key now
          // exists and where it lives — belongs to the sign-in row a few
          // lines below, which lands with its own tick and holds still long
          // enough to be read. Saying it in both places twice over, two
          // lines apart, is not emphasis; it is noise.
          bf.innerHTML = "\u2713 Signed in \u2014 picking up where your setup left off.";
          bf.hidden = false;
        }
      }
      var caughtUp = false;
      (serverSteps || []).forEach(function (x, i) {
        if (caughtUp) return;
        // On a return: adopt down to and including the sign-in, then stop.
        // The rows after it are the ones still worth watching happen.
        if (resumeAt >= 0 && i > resumeAt) { caughtUp = true; return; }
        if (/ok|skip|fail/.test(x.state)) { shown[x.id] = x.state; return; }
        if (x.state === "running") { shown[x.id] = "running"; caughtUp = true; }
      });
      // The sign-in result is on screen the moment the page paints. Start the
      // dwell clock here so it stays there: the next row cannot begin until a
      // full MIN_DWELL has passed, which is the beat the reader needs in order
      // to read that the key is theirs before the checklist moves again.
      if (resumeAt >= 0) lastFlip = Date.now();
